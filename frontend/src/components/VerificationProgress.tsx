  ...
  return (
    <div className="verification-progress" role="status" aria-live="polite">
      <div className="progress-heading">
        <span className="verify-pulse"><LoaderCircle size={24} /></span>
        <div><strong>Checking your payment</strong><span>{stages[current]}</span></div>
      </div>
      <ol>
        {stages.map((stage, index) => (
          <li className={index < current ? "done" : index === current ? "current" : ""} key={stage}>
            <span>{index < current ? <Check size={13} /> : index + 1}</span>
            {stage}
          </li>
        ))}
      </ol>
      <p>PayPruf is comparing receipt details with merchant-side records. Please keep this page open.</p>
    </div>
  );
}
